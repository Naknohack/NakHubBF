...
-- ==========================================
-- 1. KHAI BÁO SERVICES & BIẾN CỐ LÕI
-- ==========================================
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local VirtualInputManager = game:GetService("VirtualInputManager")
...
-- Khởi tạo hoặc tìm Folder chứa mục tiêu
local Enemies = workspace:FindFirstChild("Enemies") or Instance.new("Folder", workspace)
local Characters = workspace:FindFirstChild("Characters") or Instance.new("Folder", workspace)
Enemies.Name = "Enemies"
Characters.Name = "Characters"
...
-- ==========================================
-- 2. BIẾN CẤU HÌNH HỆ THỐNG (SETTINGS)
-- ==========================================
...
_G.Settings.FastAttackMode = "Fast Attack"
if _G.rz_FastAttack then _G.rz_FastAttack = nil end -- Xóa biến rác cũ
...
-- ==========================================
-- 3. CÁC HÀM HỖ TRỢ CHUNG (HELPERS)
-- ==========================================
local function IsAlive(character)
    if not character then return false end
    local hum = character:FindFirstChild("Humanoid")
...
-- ==========================================
-- 4. HỆ THỐNG FAST ATTACK (CẬN CHIẾN)
-- ==========================================
local AttackDelays = {
    ["Slow Attack"] = 0.25,
    ["Normal Attack"] = 0.15,
    ...
    ["Super Attack"] = 0.1,
    ["Extremely Fast Attack"] = 0.01,
    ["Bear Attack"] = 0.05,       -- Thêm vào theo UI của bạn
    ["Super Bear Attack"] = 0.01  -- Thêm vào theo UI của bạn
}
...
function FastAttack2:AttackNearest()
    ...
    local Part1 = ProcessFolder(Enemies)  
    local Part2 = ProcessFolder(Characters)  
...
-- ==========================================
-- 5. HỆ THỐNG GUN AURA & TỰ ĐỘNG BẮN
-- ==========================================

-- Hook Namecall để bẻ cong đạn súng
local mt = getrawmetatable(game)
local old_namecall = mt.__namecall
setreadonly(mt, false)
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if getgenv().GunAuraSettings and getgenv().GunAuraSettings.Enabled and getgenv().GunAuraSettings.CurrentTarget then  
        if method == "FireServer" or method == "InvokeServer" then  
            if tostring(self) == "RemoteEvent" or tostring(self):find("Shoot") or tostring(self):find("Attack") then  
                if type(args[1]) == "Vector3" then  
                    args[1] = getgenv().GunAuraSettings.CurrentTarget.Position  
...
-- ==========================================
-- 6. GIAO DIỆN (UI SETTINGS)
-- Chú ý: Đảm bảo biến 'FarmSettings' đã được khai báo bởi thư viện UI của bạn trước khi chạy đoạn này
-- ==========================================
...
-- ==========================================
-- 7. CÁC VÒNG LẶP NỀN (BACKGROUND LOOPS)
-- ==========================================

-- Vòng lặp Auto Click Súng
task.spawn(function()
    while true do
        pcall(performClick)
        task.wait(0.1)
...
-- Vòng lặp hiển thị mục tiêu trên UI
task.spawn(function()
...
-- Vòng lặp gây sát thương súng (Gun Aura)
task.spawn(function()
    while true do
...
-- Vòng lặp mở rộng bán kính Simulation (Hỗ trợ nhặt đồ/đánh xa)
task.spawn(function()
