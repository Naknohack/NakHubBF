...
Event:AddToggle({
    Name = "Auto tìm đảo Frozen Dimension",
    Description = "",
					...
					if not Boat then
						...
						if not plr.Character.Humanoid.Sit then
							...
							repeat
								...
								-- 🚀 Bay cao 300
								_tp(CFrame.new(Target.X, 300, Target.Z))
...
--// BIẾN
_G.AutoTikiBoat = _G.AutoTikiBoat or false
...
-- ⚙️ CONFIG
-- =========================
local TikiCFrame = CFrame.new(-16500, 50, -5000)

--// =========================
-- 🚤 HÀM LÁI THUYỀN
-- =========================
function HandleBoatToTiki()
    pcall(function()
        local char = player.Character
        if not char then return end
        ...
        if not boat or not boat:FindFirstChild("PrimaryPart") then return end
        
        -- Dừng vật lý
        boat.PrimaryPart.Velocity = Vector3.new(0,0,0)
        
        -- Bay cao 300
        boat.PrimaryPart.CFrame = boat.PrimaryPart.CFrame * CFrame.new(0,300,0)
        
        -- Bay về Tiki
        boat.PrimaryPart.CFrame = CFrame.new(
            TikiCFrame.Position.X,
            300,
            TikiCFrame.Position.Z
        )
    end)
end
