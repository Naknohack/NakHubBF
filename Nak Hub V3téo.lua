...
MiniMapGui.Parent = plr:WaitForChild("PlayerGui")
MiniMapGui.ResetOnSpawn = false

-- Frame chính
local MapFrame = Instance.new("Frame")
MapFrame.Size = UDim2.new(0, 180, 0, 180)
MapFrame.Position = UDim2.new(0.8, 0, 0.2, 0)
...
ZoomIn.Size = UDim2.new(0,25,0,25)
ZoomIn.Position = UDim2.new(1,-30,0,5)
ZoomIn.Text = "+"
ZoomIn.TextColor3 = Color3.new(1,1,1)
ZoomIn.BackgroundColor3 = Color3.fromRGB(40,40,40)
ZoomIn.Parent = MapFrame
Instance.new("UICorner", ZoomIn)

local ZoomOut = Instance.new("TextButton")
ZoomOut.Size = UDim2.new(0,25,0,25)
ZoomOut.Position = UDim2.new(1,-30,0,35)
ZoomOut.Text = "-"
ZoomOut.TextColor3 = Color3.new(1,1,1)
ZoomOut.BackgroundColor3 = Color3.fromRGB(40,40,40)
ZoomOut.Parent = MapFrame
Instance.new("UICorner", ZoomOut)
...
RunService.RenderStepped:Connect(function()
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        local x = (pos.X / MapSize) * MapFrame.AbsoluteSize.X * Zoom
        local z = (pos.Z / MapSize) * MapFrame.AbsoluteSize.Y * Zoom

        PlayerDot.Position = UDim2.new(0.5, x, 0.5, z)
    end
end)
...
local _ = Shop:AddSection({"Mua Võ"})
	...
	task.spawn(function()
		...
		if _G.BuyFly then
			pcall(function()
				...
				ReplicatedStorage.Remotes.CommF_:InvokeServer(remote,"Buy")
			end)
		end
...
Event:AddToggle({
    ...
    Description = "Auto sail and avoid sea events",
		...
		if _G.AutoFindFrozen then
			pcall(function()
				...
				if not Frozen then
					...
					if not Boat then
						local BuyBoat = CFrame.new(-16927.451, 9.086, 433.864)
						TeleportToTarget(BuyBoat)

						if (BuyBoat.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then
							replicated.Remotes.CommF_:InvokeServer("BuyBoat", _G.SelectedBoat)
						end
						...
						if not plr.Character.Humanoid.Sit then
							...
							local Target = Vector3.new(-10000000,0,37016.25)
							...
							repeat
								...
								local SeaEvent =
									CheckEnemiesBoat()
									or CheckPirateGrandBrigade()
									or CheckTerrorShark()
									or workspace:FindFirstChild("TerrorShark", true)
									or workspace.Enemies:FindFirstChild("SeaBeast")
									or workspace:FindFirstChild("HauntedShipwreck")
									or workspace:FindFirstChild("HauntedShipRaid")
								...
								_tp(CFrame.new(Target.X, Y, Target.Z))

							until not _G.AutoFindFrozen
							or workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension")
					...
					_tp(Frozen.CFrame * CFrame.new(0,500,0))
				end
...
EspPly = function()
    for _, e in next, game.Players:GetChildren() do
        pcall(function()
            if e.Character and e.Character:FindFirstChild("Head") then
                if PlayerEsp then
                    if not e.Character.Head:FindFirstChild("NameEsp" .. Number) then
                        local I = Instance.new("BillboardGui")
                        I.Name = "NameEsp" .. Number
                        I.Parent = e.Character.Head
                        ...
                        K.TextColor3 = Color3.fromRGB(0, 255, 0) -- Xanh lá
                        K.TextScaled = true
                    else
