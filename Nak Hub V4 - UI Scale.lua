...
QuestNeta = function()
	...
	local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Naknohack/RedzUi/refs/heads/main/Nak_Hub_V4_Window_MaxCapacity.lua"))():MakeWindow({
    Title = "Nak Hub V4",
...
Farm:AddDropdown({
    ...
    Options = {"Nhỏ", "Bình thường", "To", "Rất To"},
    Default = "Bình thường",
    Callback = function(p36)
        if p36 == "Nhỏ" then
            Library:SetScale(900)
        elseif p36 == "Bình thường" then
            Library:SetScale(450)
        elseif p36 == "To" then
            Library:SetScale(350)
        elseif p36 == "Rất To" then
            Library:SetScale(300)
        else
            Library:SetScale(450)
        end
    end
...
FarmSettings:AddTextBox({
    Title = "Mang theo phạm vi của đám đông",
    Description = "",
    PlaceHolder = "235",
        ...
        if _G.AutoFishing then
            pcall(function()
                ...
                local _, hitPos = workspace:FindPartOnRayWithIgnoreList(
                    Ray.new(char.Head.Position, hrp.CFrame.LookVector * MaxLaunchDistance),
                    {char, workspace.Characters, workspace.Enemies}
                )

                local castPos = hitPos and Vector3.new(hitPos.X, math.max(hitPos.Y, waterY), hitPos.Z)

                local state = tool:GetAttribute("State")
                local serverState = tool:GetAttribute("ServerState")

                -- Khi cá cắn
                if (state ~= "ReeledIn" and serverState ~= "ReeledIn") or not castPos then
...
Others:AddSection({"Những thanh kiếm bị nguyền rủa"});
    ...
    while task.wait() do
        if _G.AutoFindFrozen then
            pcall(function()
                ...
                -- ✅ Nếu đã thấy Frozen -> DỪNG HOÀN TOÀN
                if Frozen then
                    char.Humanoid.Sit = false

                    -- Stop toàn bộ velocity / tween (quan trọng)
                    ...
                    return -- 🔥 thoát luôn, không chạy tiếp _tp
                end
                ...
                if Boat and Boat:FindFirstChild("VehicleSeat") then
                    local dist = (char.HumanoidRootPart.Position - Boat.VehicleSeat.Position).Magnitude
                    if dist > 2500 then
                        Boat = nil
                    end
                end
                ...
                if not Boat then
                    ...
                    if (DealerPos.Position - char.HumanoidRootPart.Position).Magnitude <= 10 then
                        replicated.Remotes.CommF_:InvokeServer("BuyBoat", _G.SelectedBoat)
                    end
                else
    ...
    while task.wait(0.2) do
        if _G.FrozenTP then
            pcall(function()
                ...
                local hrp = char.HumanoidRootPart
                local FrozenIsland = nil

                -- 🔍 QUÉT 360° bán kính 9000
                for _, v in pairs(workspace._WorldOrigin.Locations:GetDescendants()) do
                    if v.Name == "Frozen Dimension" and v:IsA("BasePart") then
                ...
                -- ✅ Nếu tìm thấy đảo -> bay tới
                if FrozenIsland then
                    _tp(FrozenIsland.CFrame + Vector3.new(0, 50, 0))

                    -- Nếu có LeviathanGate thì mở luôn
    ...
    Callback = function(I)
        ...
        task.spawn(function()
            ...
            local function UseSkill()
                -- Giống kiểu tab farm đảo: đánh bằng skill
                Key("Z")
                task.wait(0.05)
                Key("X")
                task.wait(0.05)
            ...
            local function FlyToPart(part)
                if player.Character and part and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local function Attack(part)
                repeat
                    ...
                    if player.Character and part and part.Parent then
...
Farm:AddToggle({
	Name = "Tự động Farm 600Thông thạo ở Kiếm",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.FarmMastery_S = I
...
function TweenToMob(mob)
    ...
    local tween = TweenService:Create(
        hrp,
        TweenInfo.new(
            (hrp.Position - mob.HumanoidRootPart.Position).Magnitude / 300,
            Enum.EasingStyle.Linear
        ),
        {CFrame = mob.HumanoidRootPart.CFrame * CFrame.new(0,20,0)}
    )
...
function CheckExit()
    if workspace:FindFirstChild("Map") then
        ...
        if map:FindFirstChild("RaidMap") then
            if map.RaidMap:FindFirstChild("Finish") then
                return map.RaidMap.Finish
            end
        end
    end
end
...
function GoExit()
    local exit = CheckExit()

    if exit then
        LocalPlayer.Character.HumanoidRootPart.CFrame =
            exit.CFrame * CFrame.new(0,5,0)
    end
end
...
--// =========================
--// ⚠️ FAST ATTACK CỦA DUNGEON (CẦN XÓA)
--// =========================

--[[
XÓA TOÀN BỘ PHẦN DƯỚI NÀY
VÌ SCRIPT CHÍNH ĐÃ CÓ FASTATTACK
NẾU KHÔNG SẼ BỊ XUNG ĐỘT
]]
...
--         if mob then
--             FastAttack:Attack(mob.HumanoidRootPart)
--         end
--     end
-- end)
...
function FastAttack2:AttackNearest()
    local enemies, base = FastAttack1:GetNearestEnemies()
    if #enemies > 0 then
        self:Attack(base, enemies)
    end
end
...
function StartAttacksWithMode(mode)
    ...
    local delayTime = AttackDelays[mode] or 0.05
        ...
        while _G.Settings.FastAttack do
            ...
            if char and IsAlive(char) then
                ...
                if tool and tool.ToolTip ~= "Gun" then
                    local enemies, base = FastAttack1:GetNearestEnemies()
                    if #enemies > 0 then
                        FastAttack1:Attack(base, enemies)
                    end
                end
            end
            task.wait(delayTime)
...
FarmSettings:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            StartAttacksWithMode(_G.Settings.FastAttackMode or "Fast Attack")
        else
            StopAllAttacks()
        end
    end
...
FarmSettings:AddToggle({
    Name = "Auto Click (Melee)",
    Default = false,
    Callback = function(state)
        _G.Settings.AutoClick = state

        if state and _G.Settings.FastAttack then
            StartAttacksWithMode(_G.Settings.FastAttackMode)
        end
    end
})
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        if getgenv().GunAuraSettings.GunEnabled and curTarget and Player.Character then
            ...
            targetDisplay:SetDesc(string.format(
                "Target: %s\nHealth: %s\nDistance: %d",
                target.Name,
                health,
                dist
