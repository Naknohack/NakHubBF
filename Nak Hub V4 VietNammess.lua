...
local function GetExecutorStatus(name)
    local ex = string.lower(tostring(name))

    if ex:find("xeno") or ex:find("solara") or ex:find("krnl") then
        return "❌ Not Working"
    end
...
local ExecutorCheck = Status:AddParagraph({
    Title = "Status",
    Desc = "Checking..."
})
...
spawn(function()
    while task.wait(1) do
        ...
        local status = GetExecutorStatus(executor)

        ExecutorCheck:SetDesc(
            "Executor: " .. executor ..
            "\nStatus: " .. status
        )
    end
...
FarmSettings:AddSection({"Setting Farm"})
