...
-- Ép về mức an toàn khi mở script để tránh UI bật sẵn quá lớn
do
    local uiScale = _G.SaveData and _G.SaveData["UiScale_Save"]
    local forceNormal = {
        ["Rất to"] = true,
        ["Rat to"] = true,
        ["Rất lớn"] = true,
    ...
    if uiScale == nil or forceNormal[uiScale] then
        _G.SaveData["UiScale_Save"] = "Bình thường"
        pcall(SaveSettings)
    end
end
...
local function ApplyUiScale(option)
    if not vu32 or not vu32.SetScale then return end
    ...
    local scale = scaleMap[option] or 450
    pcall(function()
        vu32:SetScale(scale)
    end)
end

ApplyUiScale("Bình thường")
ApplyUiScale(GetSetting("UiScale_Save", "Bình thường"))
-- Criar ScreenGui
local screenGui = Instance.new("ScreenGui")
...
local FarmSettings = Library:MakeTab({
    Title = "Farm Settings",
...
Status:AddDiscordInvite({
    ...
    Invite = "https://discord.gg/kJ7bzekHcT"
})
...
Farm:AddDropdown({
    ...
    Default = GetSetting("UiScale_Save", "Bình thường"),
    Callback = function(p36)
        _G.SaveData = _G.SaveData or {}
        _G.SaveData["UiScale_Save"] = p36
        SaveSettings()
        ApplyUiScale(p36)
    end
})
...
Farm:AddToggle({
    ...
    Callback = function(v732)
        _G.AutoFactory = v732
        _G.SaveData["AutoFactory_Save"] = v732
        SaveSettings()
            ...
            pcall(function()
                ...
                if game:GetService("Workspace").Enemies:FindFirstChild("Core") then
                    for _, v734 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v734.Name == "Core" and v734:FindFirstChild("Humanoid") and v734.Humanoid.Health > 0 then
                            repeat
                                task.wait()
                            ...
                            until v734.Humanoid.Health <= 0 or not _G.AutoFactory or not v734.Parent
                        end
                    end
                else
                    topos(FactoryPos)
...
Farm:AddToggle({
    ...
    Default = GetSetting("AutoRaidPirate_Save", false),
    Callback = function(v543)
        _G.AutoRaidPirate = v543
        _G.SaveData["AutoRaidPirate_Save"] = v543
        SaveSettings()
            ...
            pcall(function()
                ...
                if (CheckPos.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 500 then
                    for _, v546 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if _G.AutoRaidPirate
                            and v546:FindFirstChild("HumanoidRootPart")
                            and v546:FindFirstChild("Humanoid")
                            and v546.Humanoid.Health > 0
                            and (v546.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 2000 then
...
Setting:AddButton({
    ...
    Description = "Cần có hướng dẫn sao lưu cho các tùy chọn ở đây",
    Callback = function()
...
FarmSettings:AddSection({"Cài đặt Farm"})
...
FarmSettings:AddSection({"Lựa chọn"})
Setting:AddTextBox({
    Title = "Phạm Vi Tấn Công Gôm Quái",
    Description = "",
    PlaceHolder = "235",
...
FarmSettings:AddTextBox({
    Title = "Chọn Chiều Cao Farm",
    Description = "",
    PlaceHolder = "20",
...
local function NotifyNightMystic(texto)
    task.spawn(function()
        local guiName = "Thông báo NightMystic"
...
spawn(function()
    while task.wait() do 
        if _G.FarmAllBoss then
            if not (_G.CurrentTargetBoss and _G.CurrentTargetBoss.Parent and _G.CurrentTargetBoss:FindFirstChild("Humanoid") and _G.CurrentTargetBoss.Humanoid.Health > 0) then
                local char = plr.Character
...
task.spawn(function()
    while task.wait() do
        if _G.AutoFindFrozen then
            pcall(function()
                -- Check xem đã có Frozen Dimension chưa
                ...
                if not Frozen then
                    ...
                    -- Nếu thuyền quá xa người chơi thì coi như không có thuyền
                    if Boat and Boat:FindFirstChild("VehicleSeat") and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                        local dist = (plr.Character.HumanoidRootPart.Position - Boat.VehicleSeat.Position).Magnitude
                        if dist > 2500 then
                            Boat = nil
                        end
                    end

                    -- Không có thuyền thì đi mua
                    if not Boat then
                        ...
                        -- Có thuyền thì lên ghế
                        if plr.Character.Humanoid.Sit == false then
                            local seatCF = Boat.VehicleSeat.CFrame * CFrame.new(0, 1, 0)
                            _tp(seatCF)
                        else
                            -- Đang ngồi thì cho thuyền chạy
                            local TargetPos = CFrame.new(-10000000, 31, 37016.25)

                            -- Gặp sea event thì bay cao hơn một chút, không thì chạy bình thường
                            ...
                            -- Nếu tìm thấy Frozen hoặc tắt toggle thì đứng dậy dừng thuyền
                            if workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension", true) or not _G.AutoFindFrozen then
                                plr.Character.Humanoid.Sit = false
                            end
                        end
                    end
                else
                    -- Đã thấy Frozen thì dừng ngay
                    plr.Character.Humanoid.Sit = false
                end
            end)
        end
...
Event:AddToggle({
    Name = "Auto Chế tạo Volcanic Magnet",
...
task.spawn(function()
    while task.wait(0.45) do
        if _G.Dungeonh and not _G.GoingExit and not _G.DeathPause and IsDungeonWorld() and IsInDungeon() then
            local alive = false
            for _, v in pairs(workspace.Enemies:GetChildren()) do
...
-- =========================================================================
-- PHẦN FIX KHỚP NỐI: FAST ATTACK & GUN AURA (KHÔNG ĐỔI LOGIC)
-- =========================================================================

-- 1. Khai báo biến hệ thống (Dùng chung cho toàn bộ script)
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local VirtualInputManager = game:GetService("VirtualInputManager")
local RunService = game:GetService("RunService")
...
-- 4. Logic Fast Attack (Melee)
local AttackDelays = {
    ["Slow Attack"] = 0.25,
    ["Normal Attack"] = 0.15,
    ["Fast Attack"] = 0.05,
    ["Super Attack"] = 0.1,
    ["Extremely Fast Attack"] = 0.01,
}

local FastAttack1 = { Distance = 100 }
function FastAttack1:Attack(BasePart, OthersEnemies)
    if not BasePart or #OthersEnemies == 0 or not RegisterAttack then return end
...
function FastAttack1:GetNearestEnemies()
    ...
    ProcessFolder(Enemies) ProcessFolder(Characters)
...
function FastAttack2:AttackNearest()
    ...
    local p1 = ProcessFolder(Enemies)
    local p2 = ProcessFolder(Characters)
    if #OthersEnemies > 0 then self:Attack(p1 or p2, OthersEnemies) end
end

-- Hàm dừng và chạy (Đã sửa tên cho khớp UI)
...
function StartFastAttack(mode)
        ...
        while _G.Settings.FastAttack do
            ...
            if char and IsAlive(char) then
                ...
                if tool and tool.ToolTip ~= "Gun" then
                    ...
                    if #enemies > 0 and basePart then FastAttack1:Attack(basePart, enemies) end
                end
            end
            task.wait(delayTime)
        end
    ...
    _G.FastAttackLoop2 = task.spawn(function()
        while _G.Settings.FastAttack do
            ...
            if char and IsAlive(char) then
                local tool = char:FindFirstChildOfClass("Tool")
                if tool and tool.ToolTip ~= "Gun" then FastAttack2:AttackNearest() end
            end
            task.wait(delayTime)
        end
    end)
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if getgenv().GunAuraSettings.GunEnabled and getgenv().GunAuraSettings.CurrentTarget then
        if method == "FireServer" or method == "InvokeServer" then
            if tostring(self):find("Shoot") or tostring(self):find("Attack") then
                if type(args[1]) == "Vector3" then args[1] = getgenv().GunAuraSettings.CurrentTarget.Position
                elseif type(args[2]) == "Vector3" then args[2] = getgenv().GunAuraSettings.CurrentTarget.Position end
            end
        end
    end
...
local function findClosestTargetForGun()
    ...
    local function CheckFolder(folder)
        ...
        for _, enemy in ipairs(folder:GetChildren()) do
            ...
            if hrp and IsAlive(enemy) and enemy ~= Player.Character then
                local dist = (hrp.Position - myPos).Magnitude
                if dist < closestDist then
                    closestDist = dist; closestTarget = enemy; closestPart = hrp
                end
            end
        end
    end
    local targetType = getgenv().GunAuraSettings.TargetType
    if targetType == "NPC" or targetType == "All" then CheckFolder(Enemies) end
    if targetType == "Player" or targetType == "All" then CheckFolder(Characters) end
...
local function applyGunDamage()
    ...
    if targetPart and ShootGunEvent then
...
local function performClick()
    ...
    if char and char:FindFirstChildOfClass("Tool") then
        ...
        if currentTime - getgenv().AutoClickShootSettings.LastClickTime >= getgenv().AutoClickShootSettings.ClickDelay then
            local clickPos = Vector2.new(Camera.ViewportSize.X / 2, 20)
...
-- 6. UI Giao diện (FarmSettings)
FarmSettings:AddDropdown({
    Name = "Tùy chỉnh tốc độ", -- Đã đổi từ "Melee Attack Mode"
    Options = {"Slow Attack", "Normal Attack", "Fast Attack", "Super Attack", "Extremely Fast Attack"},
    Default = "Fast Attack",
    Callback = function(mode) StartFastAttack(mode) end
})
...
FarmSettings:AddToggle({
    ...
    Callback = function(state)
        ...
        _G.Seriality = state
        _G.SaveData = _G.SaveData or {}
        _G.SaveData["AutoAttack_Save"] = state
        SaveSettings()

        -- Chạy chức năng
        if state then 
            StartFastAttack(_G.Settings.FastAttackMode) 
        else 
            StopAllLoops() 
        end
    end
...
FarmSettings:AddToggle({
    Name = "Bật tính năng tự động nhấp chuột",
    Default = false,
    Callback = function(state)
        _G.Settings.AutoClick = state
        if state then StartFastAttack(_G.Settings.FastAttackMode) end
    end
})

FarmSettings:AddDropdown({
    Name = "Chọn loại mục tiêu (Súng)",
    Options = TargetTypes,
    Default = "All",
    Callback = function(type) getgenv().GunAuraSettings.TargetType = type end
})

FarmSettings:AddSlider({
    Name = "Gun range",
    Min = 50, Max = 30000, Default = 500, Increment = 10,
    Callback = function(val) getgenv().GunAuraSettings.GunRange = val end
})

FarmSettings:AddToggle({
    Name = "Auto gun aura",
    Default = false,
    Callback = function(val) getgenv().GunAuraSettings.GunEnabled = val end
})

FarmSettings:AddToggle({
    Name = "Tự động nhấp chuột khi cầm súng. (2s)",
    Default = false,
    Callback = function(val)
        getgenv().AutoClickShootSettings.Enabled = val
        if not val then getgenv().AutoClickShootSettings.LastClickTime = 0 end
    end
})

local targetDisplay = FarmSettings:AddParagraph({
    Title = "Mục tiêu hiện tại",
    Content = "Searching..."
})

-- 7. Vòng lặp chính (Background Loops)
task.spawn(function()
    while true do
        pcall(performClick)
        task.wait(0.1)
...
task.spawn(function()
    while task.wait(0.5) do
        local curTarget = getgenv().GunAuraSettings.CurrentTarget
        if getgenv().GunAuraSettings.GunEnabled and curTarget then
            local target = curTarget.Parent
            local hum = target:FindFirstChild("Humanoid")
            local health = hum and string.format("%.0f%%", (hum.Health/hum.MaxHealth)*100) or "0%"
            local dist = math.floor((curTarget.Position - Player.Character:GetPivot().Position).Magnitude)
            targetDisplay:SetDesc(string.format("Target: %s\nHealth: %s\nDistance: %d studs", target.Name, health, dist))
        else
            targetDisplay:SetDesc("Searching for target...")
        end
    end
end)
...
task.spawn(function()
    while task.wait(5) do pcall(setSimulationRadius) end
end)

if NotificacaoNightMystic then
    NotificacaoNightMystic("Nak Hub", "✅")
end
