...
QuestNeta = function()
	...
	local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Naknohack/RedzUi/refs/heads/main/UiRedzHubnew_UI_Scale-1.lua"))():MakeWindow({
    Title = "Nak Hub V4",
...
local function FindRedzScaleObject()
    local coreGui = Services.CoreGui
    local gui = coreGui and coreGui:FindFirstChild("redz Library V5")
    if not gui then
        return nil
    end
    local scaleObj = gui:FindFirstChild("Scale")
...
local function ApplyUIScale(choice)
    local preset = UIScaleChoiceMap[choice] or 450
    local applied = false

    if Library and type(Library.SetScalePreset) == "function" then
        applied = pcall(function()
            Library:SetScalePreset(choice)
        end)
    end

    if not applied and Library and type(Library.SetScale) == "function" then
        applied = pcall(function()
            Library:SetScale(preset)
        end)
    end

    if not applied then
        local scaleObj = FindRedzScaleObject()
        local camera = workspace.CurrentCamera
        if scaleObj and camera then
            local computed = camera.ViewportSize.Y / math.clamp(preset, 300, 2000)
            scaleObj.Scale = computed
            applied = true
        end
    end
...
Setting:AddDropdown({
    ...
    Default = GetSetting("UI_Scale_Save", "Bình Thường"),
    Callback = function(choice)
        _G.SaveData["UI_Scale_Save"] = choice
        SaveSettings()
        ApplyUIScale(choice)
    end,
})
...
task.defer(function()
    ApplyUIScale(GetSetting("UI_Scale_Save", "Bình Thường"))
end)
