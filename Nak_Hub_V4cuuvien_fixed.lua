...
local function StopAutoFindFrozen()
    _G.AutoFindFrozen = false
    if _G.SaveData then
        _G.SaveData["FindFrozen_Save"] = false
    end
    pcall(SaveSettings)
    pcall(function()
    ...
    while task.wait() do
        if _G.AutoFindFrozen then
            pcall(function()
                ...
                if Frozen then
                    -- Gặp đảo thì dừng hẳn và tự tắt tab
                    StopAutoFindFrozen()
                    return
                end
                ...
                if not Boat then
                    ...
                    if plr.Character.Humanoid.Sit == false then
                        ...
                        -- Nếu toggle bị tắt thì đứng dậy dừng thuyền
    ...
    while wait(.1) do
        if _G.FrozenTP then
            pcall(function()
                ...
                if Frozen then
                    tp(Frozen.CFrame * CFrame.new(0, 300, 0))
                elseif workspace.Map:FindFirstChild("LeviathanGate") then
                    tp(workspace.Map.LeviathanGate.CFrame)
                    replicated.Remotes.CommF:InvokeServer("OpenLeviathanGate")
                end
            end)
        end
    end
